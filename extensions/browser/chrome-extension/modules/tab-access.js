...
export function createTabAccessPolicy({ chromeApi = chrome, isSelectedTab, getGroupColor }) {
  ...
  async function createTabWithWindowFallback(createProperties) {
    try {
      return await chromeApi.tabs.create(createProperties);
    } catch (err) {
      const win = await chromeApi.windows
        .getLastFocused({ windowTypes: ["normal"] })
        .catch(() => null);
      if (win && typeof win.id === "number") {
        return await chromeApi.tabs.create({ ...createProperties, windowId: win.id });
      }
      const created = await chromeApi.windows
  ...
  async function createTab(message, { isCurrent, attachDebugger, handoff }) {
    ...
    let tab;
    try {
      tab = await createTabWithWindowFallback({
        url: message.url,
